...
export default function BuildPage() {
    return (
        <div id="buildPage">
            <div className=" bg-classic-rose-100 py-30">
                ...
                <AppContainer className="flex flex-col gap-16 pt-10">
                    <div className="flex flex-col gap-6 text-center">
                        ...
                        <Section.Description subtitle>{LABELS.BUILD.SUBTITLE}</Section.Description>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {BuildItems.map(({ title, icon, content }, key) => (
                            <Card.Base  key={key} variant="baltic" border="gray" borderSize="sm" className="min-h-[470px] md:hover:!border-sunset-orange-500">
                                <div className="flex flex-col gap-7">
                                    <div className="relative size-16 bg-cover">
                                        <Image src={icon} alt={title} fill />
                                    </div>
                                    <Card.Title className="font-unbounded" size="md">{title}</Card.Title>
                                </div>
                                <Divider.Line className="my-6" />
                                <div>
                                    <span className="text-base text-baltic-sea-700 font-sans font-normal">{content.title}</span>
                                    <ul className=" list-disc ">
                                        {content.items.map((item, index) =>
                                            <li className="ml-5 text-base text-baltic-sea-700 font-sans font-normal" key={index}>
                                                {item}
                                            </li>
                                        )}
                                    </ul>
                ...
                <AppContainer className="flex flex-col gap-14">
                    ...
                    <div className="grid grid-cols-1 gap-16 lg:grid-cols-2 lg:gap-40">
                        ...
                        <div className="flex flex-col gap-10">
                            <Label.Paragraph className="!text-baltic-sea-300 text-base font-medium tracking-[0.18px] md:text-lg">{LABELS.BUILD.CONTRIBUTE.DESCRIPTION}</Label.Paragraph>
                            <div className="flex flex-col gap-5 items-start">
                                {LABELS.BUILD.CONTRIBUTE.LINKS.map(( { label, href }, index) => (
                                    <Link href={href} key={index} target='_blank' className="inline-flex grow-0 items-center gap-0.5 text-baltic-sea-50 hover:text-classic-rose-300">
                                        <span className="text-base underline duration-200">{label}</span>
                                        <Icons.ExternalLink  />
                                    </Link>
                                ))}
                            </div>
            ...
            <Banner title="Project ideas to explore with Bandada" description="The team has created this list of project ideas to build with Bandada, but there are many more to be discovered." >
