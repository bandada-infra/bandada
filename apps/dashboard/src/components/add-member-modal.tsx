...
export default function AddMemberModal({
  ...
  const { hasCopied, value: _clientLink, setValue: setClientLink, onCopy } = useClipboard("");
  const { address } = useAccount();
  const [signer, setSigner] = useState<any>(null);

  // Fetch the signer when the address or chain changes
  useEffect(() => {
    const fetchSigner = async () => {
      if (address) {
        try {
          const walletClient = await getWalletClient(configWallets);
          if (walletClient) {
  ...
  const addMember = useCallback(async () => {
    ...
    if (group.type === "on-chain" && group.members) {
      const existingMembers = new Set(group.members.map((memberId) => BigInt(memberId)));
      ...
      if (conflictingMembers.length > 0) {
        if (conflictingMembers.length === 1) {
          alert(`Member ID ${conflictingMembers[0]} already exists in the group.`);
        } else {
          alert(`Member IDs ${conflictingMembers.join(", ")} already exist in the group.`);
        }
        return;
      }
    }
...
${memberIds.join("\n")}
  ...
  }, [_memberIds, group, onClose, signer]);
        ...
        <ModalBody p="25px 30px">
          ...
          {group.type === "off-chain" && !group.credentials && (
            <Box position="relative" py="8">
              ...
              <AbsoluteCenter fontSize="13px" px="4" bgColor="balticSea.50">
                OR
              </AbsoluteCenter>
            </Box>
          )}
          ...
          {group.type === "off-chain" && (
            <Box mb="30px">
              <Text mb="10px" color="balticSea.800">
                {!group.credentials ? "Share invite link" : "Share access link"}
              </Text>

              <InputGroup size="lg">
                <Input
                  pr="50px"
                  placeholder={!group.credentials ? "Invite link" : "Access link"}
                  value={_clientLink}
                  isDisabled
                />
                <InputRightElement mr="5px">
                  <Tooltip label={hasCopied ? "Copied!" : "Copy"} closeOnClick={false} hasArrow>
                    <IconButton
                      ...
                      icon={<Icon color="sunsetOrange.600" boxSize="5" as={FiCopy} />}
                    />
                  </Tooltip>
                </InputRightElement>
              </InputGroup>
          ...
          <Button width="100%" variant="solid" colorScheme="tertiary" onClick={() => onClose()}>
            Close
          </Button>
        </ModalBody>
      </ModalContent>
