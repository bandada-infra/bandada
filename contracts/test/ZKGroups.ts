    ...
    describe("On-chain groups", () => {
        ...
        describe("# createOnchainGroup", () => {
            it("Should create a on-chain group if you pass the deployed semaphore contract", async () => {
                const transaction = contract
                    .connect(signers[0])
                    .createOnchainGroup(groupId, treeDepth, 0, accounts[0], 20) //20 seconds

                await expect(transaction)
                    .to.emit(semaphore, "GroupCreated")
                    .withArgs(groupId, treeDepth, 0)
                await expect(transaction)
                    .to.emit(semaphore, "GroupAdminUpdated")
                    .withArgs(groupId, constants.AddressZero, contract.address)
                await expect(transaction)
                    .to.emit(contract, "OnchainGroupAdminUpdated")
        ...
        describe("# updateOnchainGroupAdmin", () => {
            it("Should not update a on-chain group admin if the caller is not the group admin", async () => {
                const transaction = contract
                    .connect(signers[1])
                    .updateOnchainGroupAdmin(groupId, accounts[1])

                await expect(transaction).to.be.revertedWith(
                    "Caller is not the onchain group admin"
                )
            })
        ...
        describe("# addMembers", () => {
            it("Should add new members to an existing on-chain group if you pass the deployed semaphore contract", async () => {
                ...
                await contract.createOnchainGroup(
                    groupId,
                    treeDepth,
                    0,
                    accounts[0],
        ...
        describe("# updateMember", () => {
            ...
            it("Should update a member from an existing on-chain group if you pass the deployed semaphore contract", async () => {
                ...
                const { siblings, pathIndices, root } =
                    group.generateProofOfMembership(0)
                ...
                await expect(transaction)
                    .to.emit(semaphore, "MemberUpdated")
                    .withArgs(groupId, 0, members[0], newMember, root)
            })
        })
        ...
        describe("# removeMember", () => {
            ...
            it("Should remove a member from an existing on-chain group if you pass the deployed semaphore contract", async () => {
                ...
                const transaction = contract
                    .connect(signers[0])
                    .removeMember(groupId, member, siblings, pathIndices)

                await expect(transaction)
                    .to.emit(semaphore, "MemberRemoved")
                    .withArgs(groupId, 0, member, root)
            })
        })
